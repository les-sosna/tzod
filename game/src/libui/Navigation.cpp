...
std::shared_ptr<Window> UI::GetPrevFocusChild(TextureManager& texman, const InputContext &ic, const LayoutContext& lc, const DataContext& dc, Window &wnd)
{
...
std::shared_ptr<Window> UI::GetNextFocusChild(TextureManager& texman, const InputContext& ic, const LayoutContext& lc, const DataContext& dc, Window &wnd)
{
	auto focus = wnd.GetFocus();
	if (!focus && wnd.GetChildrenCount() > 0)
	{
