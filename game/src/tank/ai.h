...
//	static JobManager<GC_PlayerAI> _jobManager;
	...
	//-------------------------------------------------------------------------
	float CreatePath(vec2d from, vec2d to, int team, float max_depth, bool bTest, const AIWEAPSETTINGS *ws);
...
protected:
	...
	bool IsTargetVisible(const GC_Vehicle &vehicle, GC_RigidBodyStatic *target, GC_RigidBodyStatic** ppObstacle = NULL);
	AIPRIORITY GetTargetRate(const GC_Vehicle &vehicle, GC_Vehicle &target);

	bool FindTarget(const GC_Vehicle &vehicle, AIITEMINFO &info, const AIWEAPSETTINGS *ws);   // return true if a target was found
	bool FindItem(const GC_Vehicle &vehicle, AIITEMINFO &info, const AIWEAPSETTINGS *ws);     // return true if something was found
...
protected:
	...
	// Vp - projectile speed
	void CalcOutstrip(vec2d origin, GC_Vehicle *target, float Vp, vec2d &fake);

	void ProcessAction(const GC_Vehicle &vehicle, const AIWEAPSETTINGS *ws);

	void SetL1(aiState_l1 new_state);
	void SetL2(aiState_l2 new_state);

	void SelectState(const GC_Vehicle &vehicle, const AIWEAPSETTINGS *ws);
	void DoState(const GC_Vehicle &vehicle, VehicleState *pVehState, const AIWEAPSETTINGS *ws);
...
public:
    ...
    void OnRespawn(const GC_Vehicle &vehicle);
    void OnDie();
	...
	bool March(const GC_Vehicle &vehicle, float x, float y);
	bool Attack(const GC_Vehicle &vehicle, GC_RigidBodyStatic *target);
	bool Pickup(const GC_Vehicle &vehicle, GC_Pickup *p);
	void Stop();

    void ReadControllerState(float dt, const GC_Vehicle &vehicle, VehicleState &vs);
};
