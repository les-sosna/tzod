                               ...
                               DWORD dwNumBuffers )
{
    ...
    dsbd.lpwfxFormat     = pWaveFile->_pwfx;

    // DirectSound is only guarenteed to play PCM data.  Other
    // formats may or may not work depending the sound card driver.
    hr = _dS->CreateSoundBuffer( &dsbd, &apDSBuffer[0], NULL );
                                        ...
                                        HANDLE hNotifyEvent )
{
    ...
    if( FAILED( hr = _dS->CreateSoundBuffer( &dsbd, &pDSBuffer, NULL ) ) )
    {
                ...
                : CSound( &pDSBuffer, dwDSBufferSize, 1, pWaveFile )
{
    _dwLastPlayPos     = 0;
    _dwPlayProgress    = 0;
    _dwNotifySize      = dwNotifySize;
    _dwNextWriteOffset = 0;
    _bFillNextNotificationWithSilence = FALSE;
}
...
HRESULT CStreamingSound::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
    ...
    if( _apDSBuffer == NULL || _waveFile == NULL )
        return CO_E_NOTINITIALIZED;

    // Restore the buffer if it was lost
    BOOL bRestored;
    if( FAILED( hr = RestoreBuffer( _apDSBuffer[0], &bRestored ) ) )
        return DXTRACE_ERR( TEXT("RestoreBuffer"), hr );

    if( bRestored )
    {
        // The buffer was restored, so we need to fill it with new data
        if( FAILED( hr = FillBufferWithSound( _apDSBuffer[0], FALSE ) ) )
            return DXTRACE_ERR( TEXT("FillBufferWithSound"), hr );
        return S_OK;
    }

    // Lock the DirectSound buffer
    if( FAILED( hr = _apDSBuffer[0]->Lock( _dwNextWriteOffset, _dwNotifySize,
                                            &pDSLockedBuffer, &dwDSLockedBufferSize,
                                            &pDSLockedBuffer2, &dwDSLockedBufferSize2, 0L ) ) )
        return DXTRACE_ERR( TEXT("Lock"), hr );

    // _dwDSBufferSize and _dwNextWriteOffset are both multiples of _dwNotifySize,
    // it should the second buffer should never be valid
    if( pDSLockedBuffer2 != NULL )
        return E_UNEXPECTED;

    if( !_bFillNextNotificationWithSilence )
    {
    ...
    if( dwBytesWrittenToBuffer < dwDSLockedBufferSize )
    {
        if( !bLoopedPlay )
        {
            ...
            _bFillNextNotificationWithSilence = TRUE;
        }
        else
        {
            ...
            while( dwReadSoFar < dwDSLockedBufferSize )
            {
                ...
                if( FAILED( hr = _waveFile->Read( (BYTE*)pDSLockedBuffer + dwReadSoFar,
                                                          dwDSLockedBufferSize - dwReadSoFar,
                                                          &dwBytesWrittenToBuffer ) ) )
                    return DXTRACE_ERR( TEXT("Read"), hr );
    ...
    _apDSBuffer[0]->Unlock( pDSLockedBuffer, dwDSLockedBufferSize, NULL, 0 );
    ...
    if( FAILED( hr = _apDSBuffer[0]->GetCurrentPosition( &dwCurrentPlayPos, NULL ) ) )
        return DXTRACE_ERR( TEXT("GetCurrentPosition"), hr );

    // Check to see if the position counter looped
    if( dwCurrentPlayPos < _dwLastPlayPos )
        dwPlayDelta = ( _dwDSBufferSize - _dwLastPlayPos ) + dwCurrentPlayPos;
    else
        dwPlayDelta = dwCurrentPlayPos - _dwLastPlayPos;

    _dwPlayProgress += dwPlayDelta;
    _dwLastPlayPos = dwCurrentPlayPos;
    ...
    if( _bFillNextNotificationWithSilence )
    {
        ...
        if( _dwPlayProgress >= _waveFile->GetSize() )
        {
            _apDSBuffer[0]->Stop();
        }
    }
    ...
    _dwNextWriteOffset += dwDSLockedBufferSize;
    _dwNextWriteOffset %= _dwDSBufferSize; // Circular buffer
...
HRESULT CStreamingSound::Reset()
{
    ...
    if( _apDSBuffer[0] == NULL || _waveFile == NULL )
        return CO_E_NOTINITIALIZED;
    ...
    return _apDSBuffer[0]->SetCurrentPosition( 0L );
}
...
HRESULT CWaveFile::Open( LPCTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( _flags == WAVEFILE_READ )
    {
        if( strFileName == NULL )
            return E_INVALIDARG;
        SAFE_DELETE_ARRAY( _pwfx );

        _hmmio = mmioOpen( const_cast<LPTSTR>(strFileName), NULL, MMIO_ALLOCBUF | MMIO_READ );
        ...
        if( NULL == _hmmio )
        {
            ...
            _hmmio = mmioOpen( NULL, &mmioInfo, MMIO_ALLOCBUF | MMIO_READ );
        }
    ...
    {
        _hmmio = mmioOpen( const_cast<LPTSTR>(strFileName), NULL,
...
//       _hmmio must be valid before calling.  This function uses it to
//       update _ckRiff, and _pwfx.
//-----------------------------------------------------------------------------
HRESULT CWaveFile::ReadMMIO()
{
    ...
    if( pcmWaveFormat.wf.wFormatTag == WAVE_FORMAT_PCM )
    {
        ...
        if( NULL == _pwfx )
            return DXTRACE_ERR( TEXT("_pwfx"), E_FAIL );
...
// Desc: Resets the internal _ck pointer so reading starts from the
...
HRESULT CWaveFile::ResetFile()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if( 0 != mmioCreateChunk( _hmmio, &_ck, 0 ) )
                return DXTRACE_ERR( TEXT("mmioCreateChunk"), E_FAIL );

            if( 0 != mmioGetInfo( _hmmio, &_mmioinfoOut, 0 ) )
...
//       This uses _ck to determine where to start reading from.  So
...
HRESULT CWaveFile::Close()
{
    ...
    {
        _mmioinfoOut.dwFlags |= MMIO_DIRTY;

        if( _hmmio == NULL )
            return CO_E_NOTINITIALIZED;

        if( 0 != mmioSetInfo( _hmmio, &_mmioinfoOut, 0 ) )
            return DXTRACE_ERR( TEXT("mmioSetInfo"), E_FAIL );

        // Ascend the output file out of the 'data' chunk -- this will cause
        // the chunk size of the 'data' chunk to be written.
        if( 0 != mmioAscend( _hmmio, &_ck, 0 ) )
            return DXTRACE_ERR( TEXT("mmioAscend"), E_FAIL );

        // Do this here instead...
        if( 0 != mmioAscend( _hmmio, &_ckRiff, 0 ) )
            return DXTRACE_ERR( TEXT("mmioAscend"), E_FAIL );

        mmioSeek( _hmmio, 0, SEEK_SET );

        if( 0 != (INT)mmioDescend( _hmmio, &_ckRiff, NULL, 0 ) )
            return DXTRACE_ERR( TEXT("mmioDescend"), E_FAIL );

        _ck.ckid = mmioFOURCC('f', 'a', 'c', 't');

        if( 0 == mmioDescend( _hmmio, &_ck, &_ckRiff, MMIO_FINDCHUNK ) )
        {
            DWORD dwSamples = 0;
            mmioWrite( _hmmio, (HPSTR)&dwSamples, sizeof(DWORD) );
            mmioAscend( _hmmio, &_ck, 0 );
        }
...
HRESULT CWaveFile::WriteMMIO( WAVEFORMATEX *pwfxDest )
{
    ...
    if( 0 != mmioCreateChunk( _hmmio, &_ckRiff, MMIO_CREATERIFF ) )
        return DXTRACE_ERR( TEXT("mmioCreateChunk"), E_FAIL );
    ...
    _ck.ckid = mmioFOURCC('f', 'm', 't', ' ');
    _ck.cksize = sizeof(PCMWAVEFORMAT);
    ...
    {
        if( mmioWrite( _hmmio, (HPSTR) pwfxDest,
                       sizeof(PCMWAVEFORMAT)) != sizeof(PCMWAVEFORMAT))
    ...
    else
    {
        ...
        if( (UINT)mmioWrite( _hmmio, (HPSTR) pwfxDest,
    ...
    if( 0 != mmioCreateChunk( _hmmio, &ckOut1, 0 ) )
        return DXTRACE_ERR( TEXT("mmioCreateChunk"), E_FAIL );

    if( mmioWrite( _hmmio, (HPSTR)&dwFactChunk, sizeof(dwFactChunk)) !=
                    sizeof(dwFactChunk) )
         return DXTRACE_ERR( TEXT("mmioWrite"), E_FAIL );

    // Now ascend out of the fact chunk...
    if( 0 != mmioAscend( _hmmio, &ckOut1, 0 ) )
        return DXTRACE_ERR( TEXT("mmioAscend"), E_FAIL );
...
HRESULT CWaveFile::Write( UINT nSizeToWrite, BYTE* pbSrcData, UINT* pnSizeWrote )
{
    ...
    *pnSizeWrote = 0;
    ...
    {
        if( _mmioinfoOut.pchNext == _mmioinfoOut.pchEndWrite )
        {
            _mmioinfoOut.dwFlags |= MMIO_DIRTY;
            if( 0 != mmioAdvance( _hmmio, &_mmioinfoOut, MMIO_WRITE ) )
                return DXTRACE_ERR( TEXT("mmioAdvance"), E_FAIL );
        }

        *((BYTE*)_mmioinfoOut.pchNext) = *((BYTE*)pbSrcData+cT);
        (BYTE*)_mmioinfoOut.pchNext++;
