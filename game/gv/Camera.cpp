...
void Camera::CameraTimeStep(World &world, float dt, float scale)
{
	...
	if( vehicle )
	{
		...
		float dx = std::max(0.f, (viewSize.x - WIDTH(world.GetBounds())) / 2);
		float dy = std::max(0.f, (viewSize.y - HEIGHT(world.GetBounds())) / 2);
		...
		_target.x = std::max(r.x + dx, world.GetBounds().left + viewSize.x * 0.5f + dx);
		_target.y = std::max(r.y + dy, world.GetBounds().top + viewSize.y * 0.5f + dy);
		_target.x = std::min(_target.x, world.GetBounds().right - viewSize.x * 0.5f + dx);
		_target.y = std::min(_target.y, world.GetBounds().bottom - viewSize.y * 0.5f + dy);
	}
